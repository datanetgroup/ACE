...
void TextEditor::HandleKeyboardInputs()
{
    ...
    // Let the app know we want keys & text this frame.
    io.WantCaptureKeyboard = true;
    io.WantTextInput       = true;

    // ---- Editing / navigation keys (modern ImGuiKey API) --------------------
    if (!IsReadOnly() && ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGuiKey_Z))                { Undo();       return; }
    if (!IsReadOnly() && !ctrl && !shift &&  alt && ImGui::IsKeyPressed(ImGuiKey_Backspace))       { Undo();       return; }
    if (!IsReadOnly() && ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGuiKey_Y))                { Redo();       return; }

    if (!ctrl && !alt && ImGui::IsKeyPressed(ImGuiKey_UpArrow))                                    { MoveUp(1, shift); return; }
    if (!ctrl && !alt && ImGui::IsKeyPressed(ImGuiKey_DownArrow))                                  { MoveDown(1, shift); return; }
    if (!alt && ImGui::IsKeyPressed(ImGuiKey_LeftArrow))                                           { MoveLeft(1, shift, ctrl); return; }
    if (!alt && ImGui::IsKeyPressed(ImGuiKey_RightArrow))                                          { MoveRight(1, shift, ctrl); return; }
    if (!alt && ImGui::IsKeyPressed(ImGuiKey_PageUp))                                              { MoveUp(GetPageSize() - 4, shift); return; }
    if (!alt && ImGui::IsKeyPressed(ImGuiKey_PageDown))                                            { MoveDown(GetPageSize() - 4, shift); return; }
    if (!alt &&  ctrl && ImGui::IsKeyPressed(ImGuiKey_Home))                                       { MoveTop(shift); return; }
    if ( ctrl && !alt && ImGui::IsKeyPressed(ImGuiKey_End))                                        { MoveBottom(shift); return; }
    if (!ctrl && !alt && ImGui::IsKeyPressed(ImGuiKey_Home))                                       { MoveHome(shift); return; }
    if (!ctrl && !alt && ImGui::IsKeyPressed(ImGuiKey_End))                                        { MoveEnd(shift); return; }

    if (!IsReadOnly() && !ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGuiKey_Delete))          { Delete(); return; }
    if (!IsReadOnly() && !ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGuiKey_Backspace))       { Backspace(); return; }

    if (!ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGuiKey_Insert))                           { mOverwrite ^= true; return; }
    if ( ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGuiKey_Insert))                           { Copy(); return; }
    if ( ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGuiKey_C))                                { Copy(); return; }

    if (!IsReadOnly() && !ctrl &&  shift && !alt && ImGui::IsKeyPressed(ImGuiKey_Insert))          { Paste(); return; }
    if (!IsReadOnly() &&  ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGuiKey_V))               { Paste(); return; }

    if ( ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGuiKey_X))                                { Cut(); return; }
    if (!ctrl &&  shift && !alt && ImGui::IsKeyPressed(ImGuiKey_Delete))                           { Cut(); return; }

    if ( ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGuiKey_A))                                { SelectAll(); return; }

    if (!IsReadOnly() && !ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGuiKey_Enter))           { EnterCharacter('\n', false); return; }
    if (!IsReadOnly() && !ctrl && !alt  && ImGui::IsKeyPressed(ImGuiKey_Tab))                      { EnterCharacter('\t', shift); return; }

    // ---- Text input (UTF-16/32 queued by backends) --------------------------
...
void TextEditor::Render(const char* aTitle, const ImVec2& aSize, bool aBorder)
{
	...
	if (mHandleKeyboardInputs)
		HandleKeyboardInputs();   // sets io.WantCaptureKeyboard / WantTextInput internally

	if (mHandleMouseInputs)
		HandleMouseInputs();

	ColorizeInternal();
	Render(); // (your no-args draw routine)
